...
class camera {
    public:
        ...
        void render (const hittable& world, std::string output_filename) {
            initialize();

            std::vector<rgb_int> buffer;
            ...
            // Write the buffer to the provided file
            std::clog << "Writing image to " << output_filename << "\n";

            std::ofstream outfile;
            outfile.open(output_filename);
            outfile << "P3\n" << image_width << ' ' << image_height << "\n255\n"; // header

            for (int j = 1; j < image_height * image_width; j++) {
                outfile << buffer[j].r << ' ' << buffer[j].g << ' ' << buffer[j].b << '\n';
            }
